...
<img src="https://www.chirpstack.io/static/img/graphs/architecture.dot.png"
	title="architecture" alt="architecture" style="width:100%" />

## Generating Binaries

For Docker image require [Docker](https://docs.docker.com/install/) and [Docker Compose](https://docs.docker.com/compose/install/) to be installed.

```bash
# only (re)generate linux platform binaries
make build

# only (re)generate Windows platform (.exe) binaries
make w-build

# only (re)generate linux platform (ARM7) binaries
make arm-build

# To start code testing using Go Test Driven
make test

# To Generate All major platform binary with Tag based on .goreleaser
make dist

# only (re)generate Docker Image and export to .tar file
make docker
```
...
- Gateway Stats metadata (Bridge sends gateway stats metadata to Network Server)
